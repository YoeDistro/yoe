...
- `strace`, `vim`, `procps-ng` (ps, top, free, vmstat), `htop`, `helix`
  (modal text editor, prebuilt via `binary` class), `yazi` (terminal file
  manager, prebuilt via `binary` class)

### Bootloaders / Build tools

- `syslinux`, `meson`, `samurai`, `gawk`, `go` (toolchain bundle, prebuilt
  via `binary` class)
...
- `binary` — install prebuilt arm64/x86_64 binaries straight from upstream
  release URLs without a from-source rebuild. Backed by per-arch SHA256
  hashes and supports bare binaries (kubectl-style), multi-binary archives
  (yazi), and full toolchain bundles (Go, Helix) via `install_tree`.

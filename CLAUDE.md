...
- **One unit, one .apk; resolve variation at runtime.** A unit produces a
  ...
  configurations. Forking a unit's build flags into machine- or
  project-scoped variants is a last resort: it multiplies the cache surface,
  breaks binary reuse across projects, and pushes complexity from a few
  clean conditionals into N parallel build configurations. Reach for it only
  when runtime resolution is genuinely impossible (e.g., kernel defconfig,
  bootloader target).

...
- **`--force`/`--clean` scoped to requested recipes** — dependency recipes
  still use the cache, only explicitly named recipes are force-rebuilt.
...
- Fixed QEMU machine templates — removed UEFI firmware (`ovmf`/`aavmf`/
  `opensbi`) incompatible with MBR+syslinux boot, fixed root device
  `vda2` → `vda1`.

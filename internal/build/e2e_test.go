...
func TestE2E_DryRun(t *testing.T) {
	...
	// Should have machine from recipes-core layer
	...
	if _, ok := proj.Recipes["zlib"]; !ok {
		t.Error("expected zlib recipe from recipes-core layer")
	}

	// zlib should have been loaded via the autotools class
	if r := proj.Recipes["zlib"]; r != nil && r.Class != "package" {
		// autotools() calls package() internally, so class = "package"

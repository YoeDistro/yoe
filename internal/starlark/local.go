...
// returns any overrides declared via local(machine = "..."). Returns a
// zero-value struct (and nil error) when the file is absent.
	...
	localFn := starlark.NewBuiltin("local", func(thread *starlark.Thread, _ *starlark.Builtin, _ starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
		for _, kv := range kwargs {
			...
			case "machine":
				...
				if !ok {
					return nil, fmt.Errorf("local: machine must be a string")
				}
				captured.Machine = string(v)
			case "deploy_host":
				v, ok := kv[1].(starlark.String)
				if !ok {
					return nil, fmt.Errorf("local: deploy_host must be a string")
				}
...
func WriteLocalOverrides(projectDir string, ov LocalOverrides) error {
	path := filepath.Join(projectDir, localStarFile)
	content := "# local.star — generated by yoe; safe to delete or hand-edit.\n" +
		"# Per-developer overrides for this project. Not checked in.\n\n"
	if ov.DeployHost != "" {
		content += fmt.Sprintf("local(\n    machine = %q,\n    deploy_host = %q,\n)\n",
			ov.Machine, ov.DeployHost)
	} else {
		content += fmt.Sprintf("local(machine = %q)\n", ov.Machine)
	}

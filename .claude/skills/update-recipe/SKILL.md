...
Bump an existing recipe to the latest upstream release. Updates the version,
...
Find and read the recipe's `.star` file:
...
- Update `sha256` if the recipe uses tarball sources
...
After the recipe builds, check if packages that depend on it still build:

```bash
yoe refs <recipe-name>
```

Build any direct dependents to catch API/ABI breakage:

```bash
yoe build --force <dependent-recipe>
```
...
- Do not change configure flags without understanding why — research the
  upstream changelog and other distributions' recipes first.
